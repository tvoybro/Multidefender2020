...
void fx_NesDev(void)
{
	...
	while(nesdevFaze < 4)
	{
		...
		if ((nesdevFaze == 1)) {
			...
			if (nesdevPalId < 4 && ((nesclock & 1) == 0)) {
				++nesdevPalId;
				pal_bg_bright(nesdevPalId > 4 ? 4 : nesdevPalId);
			}
			// nesdev blink
			if (nesclock >= 128 && nesclock < 220 && (nesclock & 3) == 0) {
				if (nesdevPalId2 < 3) {
			...
			if (nesclock >= 220) {
				if (nesdevPalId3 < 4 && ((nesclock & 3) == 0)) {
